...
@IBDesignable open class KWVerificationCodeView: UIView {
    ...
    @IBInspectable public var fieldBorderWidth: CGFloat = 0{
        didSet {
            for textFieldView in textFieldViews {
                textFieldView.borderWidth = fieldBorderWidth
            }
        }
    }
    
    @IBInspectable public var fieldBorderColor: UIColor = UIColor.darkText {
        didSet {
            for textFieldView in textFieldViews {
                textFieldView.borderColor = fieldBorderColor
            }
        }
    }
    @IBInspectable public var fieldCornerRadius: CGFloat = 0 {
        didSet {
            for textFieldView in textFieldViews {
                textFieldView.cornerRadius = fieldCornerRadius
            }
        }
    }
    ...
    @IBInspectable public var textFont: String = "" {
        didSet {
            if let font = UIFont(name: textFont.trimKW(), size: textSize) {
                textFieldFont = font
            } else {
    ...
    //  public var textContentType: UITextContentType = UITextContentType.username {
    //    didSet {
    //      for textFieldView in textFieldViews {
    ...
    public func hasValidCode() -> Bool {
        for textFieldView in textFieldViews {
            if textFieldView.numberTextField.text!.trimKW() == "" {
                return false
            }
        }
    ...
    private func setupTextFieldViews() {
        ...
        for _ in 0..<requiredDigits {
            ...
            textFieldView.numberTextField.tag = index
        ...
        textFieldViews[0].isFirstIndex = true
...
extension KWVerificationCodeView: KWTextFieldDelegate {
    func isSelectedTextFieldToBeActive(index: Int) -> Bool {
        let isCorrectTextField = (index == lastActiveIndex)
        if !isCorrectTextField{
            textFieldViews[lastActiveIndex].activate()
        }
        return isCorrectTextField
